...
#%%
def score_ILRW(G,time_steps,start_node):
    '''
        ...
        for node in nx.nodes(G):
            ...
            if node != start_node:
                if node not in nx.neighbors(G,start_node):
                    ...
                    start_node_deg_div_2E = start_node_deg/double_E
                    startNode_to_node =start_node_deg_div_2E*t_steps_output[0][node]
                    
                    tempOutput = local_random_walk(time_steps,node,num_of_nodes,trans_prob_matrix)
                    node_start_deg = nx.degree(G,node)
                    node_start_deg_div_2E = node_start_deg/double_E
                    node_to_startNode = node_start_deg_div_2E*tempOutput[0][start_node]
