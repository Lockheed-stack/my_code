...
using namespace std;
vector<vector<short>> chessboard;
int board_size = 0;
...
struct NODE
{
    ...
    NODE(short loc_x, short loc_y) : x(loc_x), y(loc_y)
    {
        number = 0;
        isBackTracking = false;
    }
    NODE(short loc_x, short loc_y, const vector<NODE> list) : x(loc_x), y(loc_y), available_next_step(list)
    {
        number = 0;
        isBackTracking = false;
    }
...
//8个方向
short x_axis[8] = {-2, -2, -1, -1, 1, 1, 2, 2};
short y_axis[8] = {-1, 1, -2, 2, -2, 2, -1, 1};
...
//下一步有几种走法
short next_step_ways(short x, short y)
{
    short steps = 0;
    for (short i = 0; i < 8; i++)
    {
...
//距离中心的距离(平方)
inline short distance_to_center(short x, short y)
{
    short x_mid = board_size / 2 - 1;
    short y_mid = board_size / 2 - 1;
...
//判断最后是否回到起点
bool back_to_start(short x, short y)
{
...
void print_output(short original_x, short original_y)
{

    short all_cells = board_size * board_size;
    short length = all_cells - chessboard[original_x - 1][original_y - 1];
...
void find_next_available_jump(vector<NODE> &branch, short x, short y)
{
    for (short i = 0; i < 8; i++)
    {
        if (check(x + x_axis[i], y + y_axis[i]))
...
void find_way(short x, short y, short original_x, short original_y, short cur_loc)
{
    ...
    while (!my_stack.empty())
    {
        ...
        for (short i = top_node.number; i <= top_node.available_next_step.size();)
        {
...
int main()
{
    ...
    cin >> n >> a >> b;
    board_size = n;
    chessboard = vector<vector<short>>(n, vector<short>(n));
